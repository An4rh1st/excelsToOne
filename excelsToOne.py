...
if __name__ == "__main__":
    ...
    # for column in excel_merged:
    #     column_width = max(excel_merged[column].astype(str).map(len).max(), len(column))
    #     col_idx = excel_merged.columns.get_loc(column)
    #     writer.sheets['данные'].set_column(col_idx, col_idx, column_width)
    writer.close()
